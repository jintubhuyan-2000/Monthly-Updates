...
class DataProcessor:
    ...
    def crop(data, lat, lon):
        """Crop HEM data to a rectangle containing only finite coordinates.

        MOSDAC HEM latitude/longitude grids can contain NaN/Inf cells near
        the swath edge. Those coordinates cannot be passed to pcolormesh.
        Valid rainfall values are preserved; only the geographic plotting
        window is trimmed. Latitude/longitude values are never interpolated
        ...
        if data.shape != lat.shape or data.shape != lon.shape:
            raise ValueError(
                f"Shape mismatch: rainfall={data.shape}, latitude={lat.shape}, longitude={lon.shape}"
            )

        valid = np.isfinite(lat) & np.isfinite(lon)
        if not np.any(valid):
            raise ValueError("No finite latitude/longitude coordinates available")
        ...
        # Trim invalid rows/columns from the outside until the remaining
        # rectangle has finite coordinates everywhere.
        while r1 > r0 and c1 > c0:
            sub = valid[r0:r1, c0:c1]
            bad_rows = np.where(~np.all(sub, axis=1))[0]
            if bad_rows.size:
                r0 += int(bad_rows.min())
                r1 -= int(sub.shape[0] - 1 - bad_rows.max())
                continue

            bad_cols = np.where(~np.all(sub, axis=0))[0]
            if bad_cols.size:
                c0 += int(bad_cols.min())
                c1 -= int(sub.shape[1] - 1 - bad_cols.max())
                continue
            break

        if r1 <= r0 or c1 <= c0:
            raise ValueError("Could not find a finite rectangular geographic grid")

        d = data[r0:r1, c0:c1].copy()
        la = lat[r0:r1, c0:c1].copy()
        lo = lon[r0:r1, c0:c1].copy()

        if not np.all(np.isfinite(la)) or not np.all(np.isfinite(lo)):
            raise ValueError("Non-finite latitude/longitude remains after cropping")

        return d, la, lo

    @staticmethod
    def safe_pcolormesh(ax, lon, lat, data, **kwargs):
        """Safely plot HEM data; coordinates must be finite, rainfall may be NaN."""
        lon = np.asarray(lon, dtype=float)
        lat = np.asarray(lat, dtype=float)
        data = np.asarray(data, dtype=float)

        if lon.shape != lat.shape or lon.shape != data.shape:
            raise ValueError(
                f"pcolormesh shape mismatch: lon={lon.shape}, lat={lat.shape}, data={data.shape}"
            )
        if not np.all(np.isfinite(lon)):
            raise ValueError("Longitude contains non-finite values before pcolormesh")
        if not np.all(np.isfinite(lat)):
            raise ValueError("Latitude contains non-finite values before pcolormesh")

        # Missing rainfall is valid and should remain transparent/masked.
        z = np.ma.masked_invalid(data)
        return ax.pcolormesh(lon, lat, z, shading="auto", **kwargs)
    ...
    def _map(self,data,lat,lon,title,label,path,vmax):
        ...
        m=self.safe_pcolormesh(ax,lo,la,d,vmin=0,vmax=vmax)
...
class VisualizationTab(QWidget):
    ...
    def map_array(self,data,title,label,vmax):
        ...
        m=self.safe_pcolormesh(ax,lon,lat,d,vmin=0,vmax=max(float(vmax),1e-9)); self.figure.colorbar(m,ax=ax,pad=.02,shrink=.85).set_label(label)
