...
class DataProcessor:
    ...
    def extract_data(self, ds, variable_name=None):
        """Extract data from dataset"""
        try:
            # Auto-detect variable if not specified
            if variable_name is None:
                # Look for common variable names
                common_vars = ['sst', 'temp', 'temperature', 'data', 'value', 'SST', 'TEMP', 'Data', 'SST_DAY', 'SST_NIGHT']
                for var in common_vars:
                    if var in ds.variables:
                        variable_name = var
                        break
                if variable_name is None:
                    # Take first data variable
                    for var in ds.variables:
                        if var not in ds.dims and len(ds[var].dims) >= 2:
                            variable_name = var
                            break
            ...
            if 'lon' in ds.coords:
                ...
                # Try to find longitude dimension
                for dim in data_var.dims:
                    if 'lon' in dim.lower() or 'x' in dim.lower():
                        self.lon = ds[dim].values
                        break
    ...
    def calculate_area_average(self, data=None, lat=None, lon=None):
        ...
        try:
            ...
            print(f"Error calculating area average: {e}")
            return None
    ...
    def create_daily_accumulation_map(self, data, lat, lon, output_dir=None):
        ...
        try:
            if len(data.shape) != 3:
                print("Data must be 3D (time, lat, lon)")
                return None
            ...
            print(f"Error creating daily accumulation maps: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def create_combined_plot(self, data, lat, lon, time_avg, output_path=None):
        ...
        try:
            ...
            Mean: {np.nanmean(plot_data):.4f}
            Std Dev: {np.nanstd(plot_data):.4f}
            Min: {np.nanmin(plot_data):.4f}
            Max: {np.nanmax(plot_data):.4f}
            
            Data Points: {plot_data.size}
        ...
        try:
            ...
            for i, file_path in enumerate(file_list):
                ...
                if area_avg is not None:
                    ...
                    all_days_data.extend(area_avg if isinstance(area_avg, (list, np.ndarray)) else [area_avg])
                    if self.time is not None:
                        all_days_times.append(self.time)
                ...
                if plots_dir:
                    ...
                    fig = self.create_spatial_map(data, self.lat, self.lon, time_idx=0,
                                                 title=title, output_path=output_path)
                    if fig:
            ...
            if hasattr(self, 'data') and self.data is not None and plots_dir:
            ...
            if all_days_data:
                ...
                results['stats'] = {
                    'mean': np.nanmean(all_days_data),
                    'std': np.nanstd(all_days_data),
                    'min': np.nanmin(all_days_data),
                    'max': np.nanmax(all_days_data),
                    'count': len(all_days_data),
                    'missing': np.isnan(all_days_data).sum()
                }
                
                if len(all_days_data) > 2:
                    x = np.arange(len(all_days_data))
                    mask = ~np.isnan(all_days_data)
                    if np.sum(mask) > 2:
                        results['stats']['trend'] = np.polyfit(x[mask], all_days_data[mask], 1)[0]
            
            self.add_log(f"Processing complete! {len(results['spatial_maps'])} spatial maps generated")
            return results
    ...
    def add_log(self, message):
        """Add log message (to be connected to GUI)"""
...
class DownloadWorker(QThread):
    ...
    processing_signal = pyqtSignal(object)  # Processing results
    ...
    def run(self):
        try:
            ...
            if self.process_data and self.downloader.downloaded_files:
                ...
                # Connect log signal
                processor.add_log = self.log_signal.emit
        ...
        try:
            if 'spatial_maps' in self.results and self.results['spatial_maps']:
                ...
                ax.text(0.5, 0.5, 
                       ...
                       f"Select 'View Spatial Maps' to see\n"
                       f"the latest map in this window.",
...
class MOSDACGui(QMainWindow):
    ...
    def create_visualization_tab(self):
        ...
        self.tab_widget.setTabEnabled(2, False)  # Disabled until data is processed
        
    def create_log_tab(self):
        """Create log tab"""
        log_tab = QWidget()
        self.tab_widget.addTab(log_tab, "Processing Log")
        self.tab_widget.setTabEnabled(3, False)  # Disabled until download starts
    ...
    def start_download(self):
            ...
            if len(parts) == 4:
                try:
                    ...
                    QMessageBox.warning(self, "Invalid Bounding Box", 
                                      "Please enter a valid bounding box in format: min_lon,min_lat,max_lon,max_lat")
                    return
            else:
                QMessageBox.warning(self, "Invalid Bounding Box", 
                                  "Please enter a valid bounding box in format: min_lon,min_lat,max_lon,max_lat")
        ...
        self.downloader.downloaded_files = []  # Reset downloaded files list
...
def main():
    ...
    required_packages = ['xarray', 'netCDF4', 'matplotlib', 'scipy', 'pandas']
    missing_packages = []
