...
class MOSDACDownloader:
    ...
    def get_datasets(self) -> List[Dict[str, Any]]:
        ...
        try:
            # Try different approaches to get datasets
            ...
            if response.status_code == 200:
                ...
                # Case 1: Direct array of datasets
                ...
                # Case 2: Response has 'entries' key
                ...
                # Case 3: Response has 'datasets' key
                ...
                # Case 4: Response is a dict with dataset IDs as keys
                elif isinstance(data, dict):
                    for key, value in data.items():
                        if isinstance(value, dict):
                            dataset_id = value.get('id') or key
                            if dataset_id:
                                datasets.append({
                                    "id": dataset_id,
                                    "title": value.get('title') or value.get('name') or dataset_id,
                                    "description": value.get('description', '')
                                })
                
                # If no datasets found, return some common MOSDAC datasets as fallback
                if not datasets:
                    datasets = self.get_fallback_datasets()
                    
                return datasets
            else:
                # If API fails, return fallback datasets
                return self.get_fallback_datasets()
                
        except Exception as e:
            print(f"Error fetching datasets: {e}")
            # Return fallback datasets
            return self.get_fallback_datasets()
    ...
    def get_fallback_datasets(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            {"id": "NSCAT_L1B", "title": "NSCAT L1B", "description": "NSCAT Level 1B"},
            {"id": "MSMR_L1B", "title": "MSMR L1B", "description": "MSMR Level 1B"},
        ]
...
class MOSDACGui(QMainWindow):
    ...
    def create_download_tab(self):
        ...
        # Info label
        dataset_info = QLabel("Note: You can also type a custom dataset ID directly")
        dataset_info.setStyleSheet("color: #7f8c8d; font-size: 9pt;")
    ...
    def create_log_tab(self):
        ...
        self.tab_widget.setTabEnabled(2, False)  # Disabled until download starts
    ...
    def on_datasets_loaded(self, datasets):
        ...
        if not datasets:
            self.dataset_combo.addItem("No datasets available - you can type custom ID")
            self.dataset_combo.setItemData(0, None)  # No data for this item
        ...
        self.dataset_combo.insertSeparator(self.dataset_combo.count())
        self.dataset_combo.addItem("--- Type custom dataset ID ---", None)
            
        self.status_bar.showMessage(f"Loaded {len(datasets)} datasets")
        
        # Add a log entry if log tab is available
    ...
    def start_download(self):
        ...
        if not dataset_text or dataset_text == "No datasets available - you can type custom ID" or dataset_text == "--- Type custom dataset ID ---":
            QMessageBox.warning(self, "Input Error", 
