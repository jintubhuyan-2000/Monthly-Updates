...
class StartupSplash:
    """
    Lightweight Tkinter splash screen shown while the main application is
    being initialized. It is intentionally implemented before the heavy
    GiovanniGUI construction so users see immediate visual feedback.
    """
    ...
    def __init__(self):
        ...
        # Center splash window on the primary display.
        ...
        tk.Label(
            outer,
            text="AREA-AVERAGED TIME SERIES",
        ...
        # Progress bar.
        progress_bg = tk.Frame(outer, bg="#243B53", height=10)
        progress_bg.pack(fill="x", padx=70)
        progress_bg.pack_propagate(False)

        self.progress_fill = tk.Frame(progress_bg, bg="#5BC0EB", width=1)
        ...
        tk.Label(
            outer,
            text="Earth Observation • Remote Sensing • NASA Earthdata",
            font=("Segoe UI", 8),
            fg="#6C8096",
            bg="#0B132B"
    ...
    def update(self, message, progress):
        """Update splash status and animate the progress indicator."""
        self.status_var.set(message)
        self._target_progress = max(0, min(100, int(progress)))
    ...
    def _animate_progress(self):
        ...
        self._display_progress += 1
        self._set_progress_width(self._display_progress)

        if self._display_progress < self._target_progress:
            self._animation_job = self.root.after(8, self._animate_progress)

    def _set_progress_width(self, value):
        self.progress_fill.place_forget()

        # Get actual bar width after geometry has been calculated.
        self.root.update_idletasks()
        parent_width = max(1, self.progress_fill.master.winfo_width())
        width = max(1, int(parent_width * value / 100))
    ...
    def close(self):
        """Close the splash screen cleanly."""
        try:
...
def show_startup_splash():
    """
    Create and return the startup splash window.
    """
    splash = StartupSplash()
    splash.update("Starting NASA Giovanni...", 8)
    return splash
...
if __name__ == "__main__":
    ...
    try:
        splash.update("Loading numerical processing engine...", 18)
        time.sleep(0.08)

        splash.update("Loading data analysis libraries...", 30)
        time.sleep(0.08)

        splash.update("Loading geospatial processing engine...", 42)
        time.sleep(0.08)

        splash.update("Loading NASA Earthdata interface...", 55)
        time.sleep(0.08)

        splash.update("Preparing visualization engine...", 68)
        time.sleep(0.08)

        splash.update("Building Giovanni interface...", 82)

        # Create the actual application window.
        root = tk.Tk()

        splash.update("Configuring application workspace...", 92)

        app = GiovanniGUI(root)

        splash.update("Finalizing application startup...", 97)
        time.sleep(0.12)
        ...
        # Make sure the splash does not remain on screen if startup fails.
        splash.close()
        raise
